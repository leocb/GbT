...
function ENT:Think()
	...
	if ( self:GetOn() ) then
		...
		if(	self:GetDamageAmmount() > 0 and trace.Entity and trace.Entity:IsValid() and
			...
			LaserLib.DoDamage(	trace.Entity, trace.HitPos, trace.Normal, beamDir, self:GetDamageAmmount(), self.ply,
								self:GetDissolveType(), self:GetPushProps(), self:GetKillSound(), self.Entity );
