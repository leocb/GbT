...
------------------------------------
Coding = false
if(SinglePlayer() and Coding == false) then return end
...
function SPropProtection.HUDPaint()
	...
	if(tr.HitNonWorld) then
		if(tr.Entity:IsValid() and !tr.Entity:IsPlayer() and !LocalPlayer():InVehicle()) then
			local PropOwner = ""
			if(tr.Entity:GetNetworkedString("Owner") == "") then
				PropOwner = "Owner: N/A"
			else	
				PropOwner = "Owner: "..tr.Entity:GetNetworkedString("Owner")
			end
...
function SPropProtection.AdminPanel(Panel)
	...
	Panel:ClearControls()
	if(!SPropProtection.AdminCPanel) then SPropProtection.AdminCPanel = Panel end
	Panel:AddControl("Header", {Text = "Admin Panel - Simple Prop Protection by Spacetech"})
	...
	Panel:AddControl("Button", {Text  = "Reload Settings File", Command = "SPropProtection_ReloadSettings"})
	Panel:AddControl("Header", {Text = "Cleanup Panel"})
	for k,ply in pairs(player.GetAll()) do
		if(ply:IsValid()) then
			local NewNick = string.Replace(ply:Nick(), " ", "_")
			Panel:AddControl("Button", {Text = ply:Nick(), Command = "SPropProtection_CleanupProps "..NewNick})
		end
	end
	Panel:AddControl("Header", {Text = "Other Cleanup Options"})
	--Panel:AddControl("Button", {Text = "Cleanup Ownerless Props", Command = "SPropProtection_CleanupOwnerlessProps"})	
...
function SPropProtection.ClientPanel(Panel)
	Panel:ClearControls()
	if(!SPropProtection.ClientCPanel) then SPropProtection.ClientCPanel = Panel end
	Panel:AddControl("Header", {Text = "Client Panel - Simple Prop Protection by Spacetech"})
	Panel:AddControl("Button", {Text = "Cleanup All Props", Command = "SPropProtection_CleanupProps"})
	Panel:AddControl("Header", {Text = "Buddies Panel"})
	...
	if(table.Count(Players) == 1) then
		...
		for k,ply in pairs(Players) do
			if(ply:IsValid() and ply != LocalPlayer()) then
				local NewNick = string.Replace(ply:Nick(), " ", "_")
				local BCommand = "SPropProtection_BuddyUp_"..NewNick
...
--Fix based on hatred's post (High6)
