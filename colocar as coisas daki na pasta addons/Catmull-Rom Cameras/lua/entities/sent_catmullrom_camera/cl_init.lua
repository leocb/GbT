...
function ENT:RenderSubBeams(CTime, trackindex)
	...
	--[[
	if base ~= 0 then
		render.AddBeam(self.CatmullRomController.Spline[base], 10, CTime or 1, color_white)
	else
		render.AddBeam(self.CatmullRomController.PointsList[2], 10, CTime or 1, color_white)
	end
	--]]
...
--[[
		local is_node = i % self.CatmullRomController.STEPS -- % will return 0 if it divides perfectly into the second number
		
		if is_node == 0 then -- check to see if we're locked for edit
			local node = (i / self.CatmullRomController.STEPS) + 1 -- because the spline point list really starts at 2
			
			if self.CatmullRomController.EntityList[node].IsLocked then -- draw this segment red because this segment belongs to the locked node
				print("check red")
				col = color_red
			elseif self.CatmullRomController.EntityList[node + 1].IsLocked or
				   self.CatmullRomController.EntityList[node - 1].IsLocked then -- draw this segment yellow because we're before or after it
				print("check yellow")
				col = color_yellow
			end
		end
